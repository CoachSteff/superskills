...
def load_root_env():
    ...
    if not env_file.exists():
        print(f"❌ Root .env file not found at {env_file}")
...
def distribute_to_skill(skill_name, root_env, dry_run=False, backup=False, verbose=False):
    ...
    if not skill_dir:
        print(f"⊘ {skill_name:20} Skill directory not found")
        return False

    if skill_name not in SKILL_CREDENTIALS:
        print(f"⊘ {skill_name:20} No credential mapping defined")
        return False
    ...
    if not available_creds:
        print(f"⊘ {skill_name:20} No credentials found in root .env")
        return False

    # Show what will be distributed
    print(f"📦 {skill_name:20} {len(available_creds)}/{len(credentials_to_copy)} credentials found")

    if verbose:
        for key, value in available_creds.items():
            print(f"   → {key:30} {mask_credential(value)}")
    ...
    try:
        with open(skill_env_file, 'w') as f:
            f.write("# Auto-generated by distribute_credentials.py\n")
            f.write(f"# Skill: {skill_name}\n")
            f.write("# Source: root .env\n\n")
        ...
        print(f"   ✓ Written to: {skill_env_file}")
        return True
...
def main():
    ...
    if args.dry_run:
        ...
        print("✓ Credentials distributed successfully!")
        print()
        print("Next steps:")
